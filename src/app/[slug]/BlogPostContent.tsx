        ...
        <div className="relative h-[70vh] md:h-[85vh] w-full overflow-hidden">
          {initialPost.mainImage && (
            <div className="absolute inset-0">
              <img
                src={urlFor(initialPost.mainImage)
                  .width(1920)
                  .height(1080)
                  .url()}
            ...
            <div className="container mx-auto px-4 md:px-8 mb-16 md:mb-24">
              <div className="max-w-4xl">
                ...
                <div className="flex items-center gap-4 text-white/40 font-mono text-sm">
                  ...
                  <span>
                    {new Date(initialPost.publishedAt).toLocaleDateString(
                      "pl-PL",
                      {
                        year: "numeric",
                        month: "long",
          ...
          <div className="flex flex-col lg:flex-row gap-16">
            <aside className="lg:w-16 shrink-0 lg:sticky lg:top-32 h-fit">
              <div className="flex lg:flex-col items-center gap-6">
                ...
                {[Facebook, Twitter, Linkedin, Share2].map((Icon, i) => (
                  <button
                    key={i}
                    className="w-12 h-12 rounded-full border border-white/10 flex items-center justify-center hover:bg-[#916AFF] hover:border-[#916AFF] transition-all duration-300 group"
                  >
                    <Icon
                      size={20}
            ...
            <article className="flex-1 max-w-3xl mx-auto lg:mx-0">
              <div className="prose prose-invert prose-lg md:prose-xl max-w-none">
                <PortableText
                  value={initialPost.body}
                  components={ptComponents}
                />
              </div>

              <div className="mt-20 pt-10 border-t border-white/10 flex flex-wrap gap-4">
                <span className="text-white/40 text-sm font-bold uppercase tracking-widest mr-4">
                  Tags:
                </span>
                {[
                  "Digital",
                  ...
                  <span
                    key={tag}
                    className="text-[#916AFF] text-sm font-bold hover:underline cursor-pointer"
                  >
                    #{tag}
                  </span>
                ))}
            ...
            <aside className="lg:w-80 shrink-0 hidden xl:block sticky top-32 h-fit">
              <h4 className="text-xs font-black uppercase tracking-[0.2em] text-white/40 mb-8">
                Polecane artykuły
              </h4>
          ...
          <section className="xl:hidden container mx-auto px-4 md:px-8 mt-24 pt-24 border-t border-white/5">
            ...
            <div className="grid md:grid-cols-2 gap-8">
              {relatedPosts.map((rp) => (
                ...
                >
                  <img
                    src={urlFor(rp.mainImage).width(800).url()}
                    alt={rp.title}
                  ...
                  <div className="absolute inset-0 bg-linear-to-t from-black/80 to-transparent flex flex-col justify-end p-6">
                    <h5 className="text-xl font-bold text-white group-hover:text-[#916AFF] transition-colors">
                      {rp.title}
                    </h5>
                  </div>
                </Link>
