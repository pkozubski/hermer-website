    ...
    const handleScroll = () => {
      ...
      rafId = requestAnimationFrame(() => {
        ...
        if (leftColumnRef.current) {
          leftColumnRef.current.style.transform = `perspective(1500px) rotateY(12deg) translateZ(-20px) translateY(${-offset}px)`;
        }
        if (rightColumnRef.current) {
          rightColumnRef.current.style.transform = `perspective(1500px) rotateY(-12deg) translateZ(-20px) translateY(${offset}px)`;
        }
      });
    };
