...
interface FaqProps {
  items?: FaqItem[];
  sanitySlug?: string;
  /** When true, skip the client-side fetch — items are already server-fetched */
...
export const Faq: React.FC<FaqProps> = ({ items = FAQ_ITEMS, sanitySlug, skipClientFetch = false }) => {
  ...
  useEffect(() => {
    ...
    async function loadFaqFromSanity() {
      if (!slug || skipClientFetch) {
        return;
      }

      try {
        const response = await fetch(
          `/api/faqs?slug=${encodeURIComponent(slug)}`,
          {
            cache: "no-store",
          },
        );
  ...
  return (
    <section
      ref={sectionRef}
      className="py-24 lg:py-32 bg-transparent relative overflow-clip"
    >
      <JsonLdScript
        data={{
            ...
            acceptedAnswer: {
              ...
              text:
                typeof item.answer === "string"
                  ? item.answer
                  : extractTextFromPortableText(item.answer),
            },
          })),
        }}
      />
      <div className="container mx-auto px-4 sm:px-8 lg:px-16 relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-24 items-start">
          ...
          <div
            className="hidden lg:block lg:col-span-7 pt-8 min-h-[300px] sticky top-32 self-start"
            style={{
              opacity: isInView ? 1 : 0,
          ...
          >
            {activeItem && (
              <div
                key={activeItem.id}
                className="animate-[faqFade_0.35s_ease-out]"
              >
