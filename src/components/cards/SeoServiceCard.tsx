...
// Imported SEO Sub-components
import KeywordRankings from "./seo-service/KeywordRankings";
import TrafficGraph from "./seo-service/TrafficGraph";
import AuditScore from "./seo-service/AuditScore";
import LinkNetwork from "./seo-service/LinkNetwork";
...
interface SlideData {
  type: "keywords" | "traffic" | "audit" | "links";
  title: string;
  subtitle: string;
...
const SLIDES: SlideData[] = [
  {
    ...
    subtitle: "Pierwsza strona Google dla Twoich fraz",
    theme: "white",
  },
  {
    type: "traffic",
    title: "Wzrost Ruchu",
    subtitle: "Organiczni klienci bez płacenia za klik",
    theme: "black",
  },
  {
    type: "audit",
    title: "Techniczne SEO",
    subtitle: "Optymalizacja Core Web Vitals",
    theme: "glass",
  },
  {
    type: "links",
    title: "Link Building",
    subtitle: "Budowanie autorytetu domeny",
    theme: "glass",
  },
];
...
export function SeoServiceCard({
  className,
  title = "Pozycjonowanie SEO",
  description = "Kompleksowa strategia widoczności. Od doboru słów kluczowych, przez optymalizację techniczną, aż po skuteczny link building. Bądź tam, gdzie szukają Cię klienci.",
}: ServiceCardProps) {
          ...
          >
            {SLIDES.map((slide, index) => (
              <Slide
                ...
                onClick={() => goTo(index)}
              >
                {/* Render decor only for glass theme to keep others clean */}
                {slide.theme === "glass" && (
                  <>
                ...
                <SlideInner>
                  ...
                  <ContentArea>
                    {slide.type === "keywords" && <KeywordRankings />}
                    {slide.type === "traffic" && <TrafficGraph />}
                    {slide.type === "audit" && <AuditScore />}
                    {slide.type === "links" && <LinkNetwork />}
                  </ContentArea>
                </SlideInner>
              </Slide>
            ))}
...
const DecorBlobs = styled.div<{ $type: string }>`
  ...
  &::after {
    ...
    background: ${(props) =>
      props.$type === "audit"
        ? "radial-gradient(circle, rgba(146, 64, 14, 0.3), transparent 70%)"
        : "radial-gradient(circle, rgba(30, 58, 138, 0.4), transparent 70%)"};
  }
`;
